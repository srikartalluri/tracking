...
def constructBayesNet(gameState: hunters.GameState):
    ...
    w = list(range(X_RANGE + Y_RANGE + 6))

    variables = [PAC, GHOST0, GHOST1, OBS0, OBS1]
    edges = [(GHOST0, OBS0), (PAC, OBS0), (GHOST1, OBS1), (PAC, OBS1) ]
    variableDomainsDict = {PAC: e, GHOST0:e, GHOST1:e, OBS0:w, OBS1:w}
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        l = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        rest = l
        for curElim in eliminationOrder:
            rest, cur = joinFactorsByVariable(rest, curElim)

            if len(cur.unconditionedVariables()) <= 1:
                continue
        
            rest.append(eliminate(cur, curElim))
        
        return normalize(joinFactors(rest))
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        for key in self.keys():
            self[key] = self.__getitem__(key)/t
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if (manhattanDistance(ghostPosition, jailPosition) == 0 and noisyDistance is None):
            return 1
        ...
        #busters.getObservationProbability()
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            newPoss = self.getPositionDistribution(gameState,oldPos)

            for curKey in newPoss:
                c[curKey] += newPoss[curKey] * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            self.particles += [self.legalPositions[i % l]]
        # base = self.numParticles // l

        # leftOver = self.numParticles % l

        # self.particles = [base] * (l - leftOver) + [base + 1] * leftOver
    ...
    def getBeliefDistribution(self):
        ...
        #     curPos = self.legalPositions[i]
        #     curParticles = self.particles[i]

        #     d[curPos] = curParticles
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self.particles += [positions[i % len(positions)]]
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                w *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), p[i], self.getJailPosition(i))
            weights[p] += w
        weights.normalize()
