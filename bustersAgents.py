...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        # if len(livingGhostPositionDistributions) == 3:
        #     print(livingGhosts)
        ...
        for i in range(len(livingGhostPositionDistributions)):
            # if not livingGhosts[i]:
            #     continue
                
            maxPos = (0,0)
            maxProb = 0

            curDist = livingGhostPositionDistributions[i]
            for curKey in curDist:
                if curDist[curKey] > maxProb:
                    maxProb = curDist[curKey]
                    maxPos = curKey
        ...
        for curGhost in ghostPos:
            d = self.distancer.getDistance(curGhost, pacmanPosition)
            if d < minDist:
                minDist = d
                closestGhost = curGhost
        
        bestMove = legal[0]
        minD = 999999999999999
        for action in legal:
            suc = Actions.getSuccessor(pacmanPosition, action)
            d = self.distancer.getDistance(suc, closestGhost)
            if d < minD:
                bestMove = action
                minD = d
